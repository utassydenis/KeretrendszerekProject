...
public class CharacterManagerImpl implements CharacterManager{
    ...
    private static CharactersEntity convertCharactersModel2Entity(Characters character) {
        return CharactersEntity.builder()
                ...
                .Abbreviation(character.getAbbrev())
                .description(character.getDescription())
                .build();
    }

    @Override
    public Characters record(Characters character) throws CharactersAlreadyExistsException {
        if (characterRepository.findById(character.getId()).isPresent()) {
            throw new CharactersAlreadyExistsException();
        }
    ...
    @Override
    public Characters readById(int id) throws CharactersNotFoundException {
        Optional<CharactersEntity> entity = characterRepository.findById(id);
        if (entity.isEmpty()) {
            throw new CharactersNotFoundException(String.format("Cannot find character with ID: %s", id));
        }
