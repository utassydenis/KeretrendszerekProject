...
import hu.uni.eku.tzs.service.WorksManager;
...
public class WorksController {
    ...
    private final WorksManager worksManager;
    ...
    public WorksDto create(@RequestBody WorksDto worksDto) {
        Works work = worksMapper.workDto2workss(worksDto);
        try {
            Works recordedWork = worksManager.record(work);
            return worksMapper.works2workDto(recordedWork);
    ...
    @GetMapping("/{id}")
    public WorksDto readById(@PathVariable int id) throws WorksNotFoundException {
        try {
            return worksMapper.works2workDto(worksManager.readById(id));
    ...
    public WorksDto modify(@RequestBody WorksDto dto) {
        Works work = worksMapper.workDto2workss(dto);
        try{
            Works modifyWork = worksManager.modify(work);
            return worksMapper.works2workDto(modifyWork);
        } catch (WorksNotFoundException e){
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, e.getMessage());
        }
    }
